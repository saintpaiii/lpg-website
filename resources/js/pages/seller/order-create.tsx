...
const breadcrumbs: BreadcrumbItem[] = [
    ...
    { title: 'New Order', href: '/seller/orders/create' },
];
...
function CustomerCombobox({
    ...
    function select(c: Customer) { onChange(c.id.toString()); setQuery(''); setOpen(false); }
                ...
                <div className="flex items-center justify-between rounded-md border border-input bg-background px-3 py-2 text-sm">
                    <div>
                        ...
                        {(selected.barangay || selected.city) && (
                            <p className="text-xs text-gray-400 mt-0.5">{[selected.barangay, selected.city].filter(Boolean).join(', ')}</p>
                        )}
                    </div>
                    ...
                    {filtered.length === 0 ? (
                        ...
                        filtered.map((c) => (
                            ...
                            >
                                ...
                                <p className="text-xs text-gray-400">{[c.phone, c.barangay, c.city].filter(Boolean).join(' · ')}</p>
                            </button>
                        ))
                    )}
                </div>
...
export default function SellerOrderCreate({ customers, products }: Props) {
    ...
    // Price per unit depends on transaction type
    function getPrice(product: ProductOption) {
        return data.transaction_type === 'refill' ? product.refill_price : product.purchase_price;
    }
    ...
    function updateLine(idx: number, field: keyof LineItem, val: string) {
        const updated = data.items.map((item, i) => i === idx ? { ...item, [field]: val } : item);
        ...
        if (field === 'product_id') {
            setStockErrors((prev) => { const next = [...prev]; next[idx] = ''; return next; });
        }
    }
    ...
    const total = data.items.reduce((sum, line) => {
        ...
        return sum + (product ? getPrice(product) * (parseInt(line.quantity) || 0) : 0);
    }, 0);
            ...
            <div className="flex flex-1 flex-col gap-6 p-6 max-w-3xl">
                ...
                <div>
                    ...
                    <p className="mt-0.5 text-sm text-gray-500">
                        Create a new customer order. Stock is deducted when the order is confirmed.
                    </p>
                </div>
                ...
                <form onSubmit={handleSubmit} className="grid gap-6">
                        ...
                        <CardContent className="grid gap-4">
                            <div className="grid gap-4 sm:grid-cols-3">
                                <div className="grid gap-1.5">
                                    ...
                                    <Select value={data.transaction_type} onValueChange={(v) => setData('transaction_type', v as 'refill' | 'new_purchase')}>
                    ...
                    <Card>
                        ...
                        <CardContent className="grid gap-3">
                            ...
                            {data.items.map((line, idx) => {
                                ...
                                const subtotal = price * (parseInt(line.quantity) || 0);
                                ...
                                return (
                                    <div key={idx} className="grid grid-cols-[1fr_96px_100px_32px] gap-2 items-start">
                                                ...
                                                {products
                                                    ...
                                                    .map((p) => (
                                                        <option key={p.id} value={p.id.toString()} disabled={p.stock === 0}>
                                                            {p.name}{p.brand ? ` (${p.brand})` : ''} — ₱{getPrice(p).toLocaleString()} | {p.stock} left
                                                        </option>
                                                    ))}
                                            </select>
                                            {product && (
                                                <p className="mt-0.5 px-1 text-[10px] text-gray-400">
                                                    ₱{price.toLocaleString()} each · {product.stock} available
                                                </p>
                                            )}
                                            {stockErrors[idx] && (
                                        ...
                                        <div className="flex h-9 items-center justify-end text-sm font-medium text-gray-700">
                                            {product ? `₱${subtotal.toLocaleString('en-PH', { minimumFractionDigits: 2 })}` : '—'}
                                        </div>
                    ...
                    <div className="flex justify-end gap-3">
                        <Button type="button" variant="outline" onClick={() => router.visit('/seller/orders')}>Cancel</Button>
                        <Button type="submit" disabled={processing || !canSubmit} className="bg-blue-600 hover:bg-blue-700 text-white">
